...
- `feature/partner-<slice>`：以后每个较小功能的短期分支。
- `integration/f146-upstream-<date>`：临时验证某次上游同步，验证完成后再并回 F146。
...
git switch -c integration/f146-upstream-YYYYMMDD
...
git switch feature/f146-partner-plugin-library
git merge --ff-only integration/f146-upstream-YYYYMMDD
```
