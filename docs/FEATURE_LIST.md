...
| F130 | Partner Composer-First Skill Workspace: editable scene prompts and real Skills, plus Task materials/Collaboration/Artifacts cards and a tabbed detail workspace                                        | Enhancement            | High     | `v0.1.64`           | InProgress  | Existing KodaX SkillRegistry, `skill` tool, `skillDynamicContext`, Partner AgentProfile/tool policy, and F095/F109/F114/F122-F124; no new SDK API and no permission expansion                                                                                                                                                                                                                                                        | [v0.1.64](features/v0.1.64.md#feature_130-partner-composer-first-skill-workspace)                                          |
...
| ID   | Outcome                                                                                                                                               | Category          | Priority | Target      | Status     | SDK gate                                                                                                                                                                                                                             | Design                                                                                                                                          |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | -------- | ----------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| F146 | Partner Plugin Library Extension: independently installable UI package, persistent experts, and session-scoped document connectors (Feishu CLI first) | New / Enhancement | High     | v0.1.61-p.1 | InProgress | Reuse public agentProfile and existing single-Skill invocation; Partner direct-create uses trusted receipts plus an independent content-verification signal and a unified typed detail workspace; existing MCP transport unchanged; no SDK fork, KodaX Runtime change, Coder change, or Skill-system rewrite | [F146 design](features/v0.1.61.md#feature_146-partner-plugin-library-extension), [development plan](features/v0.1.61-partner-plugin-library.md) |
...
| `v0.1.61-p.1`     | Fork-only Partner plugin prerelease                                                                                   | F146 (fork)                       | May ship the Partner host capability independently; it is not canonical v0.1.61 and does not mark F137/F139 complete.                                                                                                                                                                                                                                                                     |
| `v0.1.61`         | Native document Skills, semantic UI, and merged Partner plugins                                                       | F137, F139, F146 after merge      | Canonical release waits for all document, semantic UI, Partner, accessibility, delivery, and validation gates.                                                                                                                                                                                                                                                                            |
| `v0.1.62`         | —                                                                                                                     | none                              | —                                                                                                                                                                                                                                                                                                                                                                                         |
| `v0.1.63`         | —                                                                                                                     | none                              | —                                                                                                                                                                                                                                                                                                                                                                                         |
| `v0.1.64`         | Partner composer-first Skill workspace                                                                                | F130                              | New tasks offer editable scene prompts and governed Skills; Task materials/Collaboration/Artifacts live in a compact card rail, while confirmations are conditional and selected details plus real Files/Browser/Terminal open in a tabbed workspace.                                                                                                                                     |
| `v0.1.65`         | —                                                                                                                     | none                              | —                                                                                                                                                                                                                                                                                                                                                                                         |
...
- Status: InProgress; Created: 2026-08-31; Updated: 2026-09-03; Category: New / Enhancement; Priority: High.
- Target: fork-only prerelease v0.1.61-p.1, using the v0.1.61 planning file without publishing canonical v0.1.61 ahead of F137/F139. Current package/SDK versions remain the development baseline; Released: none.
- Outcome: Partner-only Plugins entry with Experts and Connectors; durable per-session expert roles and explicit connector connections; real extension installation/removal; destination-faithful remote delivery that opens verified resources directly while retaining review only for operations whose risk requires it.
- Confirmed boundary: the Skill library and invocation remain shared infrastructure, but P8 does not modify the shared `feishu-office-suite` Skill or its lock/source records. Experts may configure zero or one existing Skill in the first release; no new Skill resolver, installer, matching engine or shared-registry mutation.
- Scope correction: fork-specific F146 adds the requested minimal Partner UI host. The current P8 slice is limited to Partner's provider-neutral remote-resource delivery contract and right-side web session; provider connectors, provider write actions and expert definitions are separate follow-ups. It does not mark the broader F096 connector roadmap or shelved F102 general slot registry complete. No Coder-specific prompt, UI, runtime or business logic is an implementation target.
- 2026-09-03 correction: P9 replaces the strict Markdown-source round-trip gate with a receipt-first resource contract, removes Partner's permanent Pending-review / Results destinations, and routes task materials, collaboration and outputs into one typed right-side detail workspace. KodaX Runtime and Coder remain unchanged.
- P9 implementation: completed and independently reviewed. Trusted remote receipts now surface their canonical URL before non-blocking content verification; Partner's three task cards route directly into the unified browser/file/artifact/Skill/expert/connector detail workspace. Automated gates: 3,766 passed / 4 skipped / 0 failed, plus typecheck, lint, renderer production build and diff check.
...
- Historical P4/P5 baseline: user selected official Feishu CLI; the bounded host adapter, per-session document consent, remote snapshots, reviewed create/append proposals and receipts are implemented. P8 supersedes the model route for future low-risk creates but preserves these records and the reviewed append path. Contract and verification boundaries: [Feishu slice](features/f146-p4-p5-feishu.md). P6 still requires user-authorized real-service validation. No third-party authorization or remote document write has been performed.
- Historical onboarding follow-up: WorkBuddy-inspired minimal cards/dialog and composer toggles using Space styles, explicit private CLI installation and cancellable official onboarding; library 0.5.0. [Contract, reviews and verification limits](features/f146-feishu-onboarding.md). Real-service authorization and the locked-Mac visual check remain unverified; P8 replaces only the new-document review requirement.
- Latest Feishu evidence supersedes the historical unverified-onboarding note above: [onboarding sections 6–7](features/f146-feishu-onboarding.md) record the user-completed connection and 0.5.1 horizontal-card upgrade; remote document read/write validation remains separate.
- 2026-09-01 expansion: user approved the [official route matrix and batches](features/f146-multi-connectors.md). First batch adds private official CLI onboarding and exact-resource reads for WeCom, DingTalk and Tencent Meeting, library 0.6.0, macOS arm64 only. The other six providers and new remote writes remain pending; personal KDocs requires app/server prerequisites. Real-service account acceptance remains distinct from fixture tests.
- 2026-09-02 P8 scope-corrected increment: Partner now has a provider-neutral `providerId + verified HTTPS canonical URL` delivery contract, single-open task projection, and an isolated persistent right-side web session. Any present or future connector can hand off a verified remote resource without adding its provider name or URL grammar to Partner core; the platform's official login page and resource remain in the same right-side tab. The generic success path does not require a local Artifact, Results detour or Pending-review card. Platform-specific creation, account selection, API read-back, connector manifests and expert prompts are explicitly deferred and are not acceptance gates for this slice. Coder does not consume the capability. See the [P8 plan](features/v0.1.61-partner-plugin-library.md#p8partner-通用平台原生资源直达交付2026-09-02).
- 2026-09-03 P8 integration correction: Partner's new-document path is now single-source. Model tool, mutation IPC and service proposal input accept append only; the old create-proposal opener is removed, historical create proposals remain migration-readable but are hidden and reject without dispatch. Verified native document tasks enter 任务产物; both the card preview and its full list open/reuse the canonical provider page in the Partner right sidebar. Feishu Markdown read-back now recognizes the official `<title>` envelope and exporter-added blank lines, preventing successful creates from being mislabeled `partial`. Coder remains unchanged.
